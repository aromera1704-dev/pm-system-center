...
export function Sidebar() {
  ...
  return (
    <div className="fixed left-0 top-0 h-screen w-20 bg-white border-r border-gray-200 flex flex-col items-center py-8 z-50 shadow-sm">
      {/* Logo */}
      <div className="mb-12 flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-[#1a5a6a] to-[#b87333] shadow-lg">
        ...
        {navItems.map((item, index) => (
          <button
            ...
            className={`group relative flex items-center justify-center w-12 h-12 rounded-xl transition-all duration-300 ${
              item.active
                ? 'bg-[#1a5a6a]/10 text-[#1a5a6a] shadow-sm'
                : 'text-gray-500 hover:text-[#1a5a6a] hover:bg-gray-50'
            }`}
            title={item.label}
          >
      ...
      {/* Settings */}
      <button className="flex items-center justify-center w-12 h-12 rounded-xl text-gray-500 hover:text-[#1a5a6a] transition-all duration-300 hover:bg-gray-50">
